# QuickDmail — HeyGen Gmail Collector

> Chrome 扩展 · Manifest V3 · 版本 1.4.1

自动捕获 Gmail 中 HeyGen 的验证邮件链接，并集成 DuckDuckGo 私密邮箱地址生成，让批量注册账号的全流程在一个面板内完成。
...
- **实时监控**：后台持续监听 Gmail 收件箱，HeyGen 验证邮件到达时立即提取 `magic-web` 链接
...
- 自动整理所有出现过的 HeyGen 注册邮箱，记录首次 / 最后收件时间及次数
- 支持关键词搜索、按时间过滤（某日期之前 / 时间段内）
- 批量勾选账号，一键**轮转复制**邮箱地址（配合批量注册）
- 导出账号列表为 CSV
- 可同步到 **Google Sheets**（需配置 OAuth Client ID）

### 🐥 Duck 标签页 — DuckDuckGo 私密邮箱

解决"注册需要邮箱但不想暴露真实地址"的问题，配合 HeyGen 注册使用。
...
| **链接发光按钮** | 用某个 Duck 地址注册 HeyGen 后，当验证邮件到达时，该地址行右侧的链接按钮**自动变绿发光**，点击即可复制验证链接 |
...
- **Google Sheets 同步**：配置表格 URL，自动或手动同步账号数据
- **自动移入垃圾桶**：非验证类 HeyGen 邮件可选择自动归档
...
2. 用该地址注册 HeyGen 账号
3. HeyGen 发验证邮件 → DDG 转发到你的真实 Gmail
4. 插件自动捕获验证链接
...
| `content.js` | Gmail 页面注入脚本：DOM 监听、Gmail 已读标记 |
| `page-interceptor.js` | Gmail MAIN world 脚本：拦截 XHR/fetch，提取验证链接和 Thread ID |
...
| `background.js` | Service Worker：消息路由、Google Sheets 同步、徽章更新 |
| `popup.html/js` | 插件弹窗 UI：四个标签页全部功能 |
...
| 权限 | 用途 |
|------|------|
| `storage` | 保存捕获的链接、账号、Duck 地址 |
| `tabs` / `scripting` | 向 Gmail 页面注入脚本、执行已读标记 |
| `activeTab` | 获取当前标签页信息 |
| `identity` | Google OAuth 授权（Sheets 同步） |
| `notifications` | 新链接到达时系统通知 |
| `https://mail.google.com/*` | 读取 Gmail 页面 |
